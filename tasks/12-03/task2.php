...
$content = getFileContent("files/t2_task.txt");
...
foreach ($content as $s) {
    ...
    foreach ($s as $index => $val) {
        ...
        $d = isset($s[$index]) ? $s[$index] : null; #m
        $o = isset($s[$index + 1]) ? $s[$index + 1]  : null; #u
        $n = isset($s[$index + 2]) ? $s[$index + 2] : null; #l
        $op = isset($s[$index + 3]) ? $s[$index + 3] : null; #l
        $t = isset($s[$index + 4]) ? $s[$index + 4] : null; #l
        $db1 = isset($s[$index + 5]) ? $s[$index + 5] : null;
        $db2 = isset($s[$index + 6]) ? $s[$index + 6] : null;

        $do = "{$d}{$o}{$n}{$op}";
        $dont = "{$d}{$o}{$n}{$op}{$t}{$db1}{$db2}";

        // if ($do === "do()" || $dont == "don't()") {
        //     var_dump($do, $dont) . PHP_EOL;
        // }
